        ...
        <?php foreach ($bloques as $i => $bloque): ?>
          <tr>
            <td>
              <div style="display:flex;gap:0.35rem;">
                <form method="post" action="/admin/contenido/<?= (int) $bloque['id'] ?>/mover">
                  ...
                  <button type="submit" class="btn btn-secundario" style="padding:0.4rem 0.7rem;" <?= $i === 0 ? 'disabled' : '' ?> aria-label="Subir">&uarr;</button>
                </form>
                <form method="post" action="/admin/contenido/<?= (int) $bloque['id'] ?>/mover">
                  <?= \App\Helpers\Csrf::field() ?>
                  <input type="hidden" name="direccion" value="abajo">
                  <button type="submit" class="btn btn-secundario" style="padding:0.4rem 0.7rem;" <?= $i === count($bloques) - 1 ? 'disabled' : '' ?> aria-label="Bajar">&darr;</button>
                </form>
              </div>
            </td>
            ...
            <td class="admin-acciones">
              ...
              <form method="post" action="/admin/contenido/<?= (int) $bloque['id'] ?>/visible" style="display:inline;">
                <?= \App\Helpers\Csrf::field() ?>
