...
<section class="seccion contenedor" style="max-width:640px;">
  ...
  <?php if ($enviado): ?>
    <?php if ($yaExistia): ?>
      <p style="margin-top:1rem;">Ya registramos tu resena anteriormente. ¡Gracias por compartir tu experiencia!</p>
    <?php else: ?>
      <p style="margin-top:1rem;">Gracias por tu resena. Sera publicada en la ficha del paquete despues de una breve revision.</p>
    <?php endif; ?>
  <?php else: ?>
    ...
    <form method="post" action="/resena/<?= htmlspecialchars($codigo, ENT_QUOTES, 'UTF-8') ?>">
      ...
      <div class="campo">
        ...
        <?php if (!empty($errores['calificacion'])): ?><small style="color:var(--color-error);"><?= htmlspecialchars($errores['calificacion'], ENT_QUOTES, 'UTF-8') ?></small><?php endif; ?>
      </div>
      ...
      <div class="campo">
        ...
        <?php if (!empty($errores['comentario'])): ?><small style="color:var(--color-error);"><?= htmlspecialchars($errores['comentario'], ENT_QUOTES, 'UTF-8') ?></small><?php endif; ?>
      </div>

      <button type="submit" class="btn btn-primario" style="width:100%;">Enviar resena</button>
    </form>
  <?php endif; ?>
</section>
